...
-(void)viewWillAppear:(BOOL)animated{
    HomeTable.frame=CGRectMake(0, 64, kScreenWidth, kScreenHeight-49);
   ...
   // NSLog(@"%@",req.URL);
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:req completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        for (int i=0; i<tpye.count; i++) {
            ...
            if ([tpye[i] intValue]==0) {
                [hmodel yy_modelSetWithDictionary:[dic valueForKey:@"article"][i]];
                hmodel.ulr=[[[dic valueForKey:@"article"][i]valueForKey:@"topImage"]valueForKey:@"url"];
                hmodel.resource=[[dic valueForKey:@"article"][i]valueForKey:@"resource"];
